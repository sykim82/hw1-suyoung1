  ...
  public synchronized void processCas(CAS aCAS) throws ResourceProcessException {
    ...
    while (annotationIter.hasNext()) {
      ...
      try {
        ...
        if(annot.getType().getName().equals("MyID")){
          sentenceID=aText;
          sentenceOff = annot.getEnd()+1;
        }
        if(annot.getType().getName().equals("MyGene")){
          
          gene=aText;
          fileWriter.write(sentenceID +"|" +(annot.getBegin()-sentenceOff)+ " "+(annot.getEnd()-sentenceOff)+"|"  + gene + "\n");
